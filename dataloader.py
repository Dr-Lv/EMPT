...
class FewshotDataset(data.Dataset):
    def __init__(self,file_name,N,K,Q,noise_rate,ins_per_class,device=1):
        ...
        if ins_per_class == -1:
            ...
            num_classes = 5
        self.classes = list(self.json_data.keys())[:num_classes]
        self.N, self.K, self.Q = N,K,Q
        self.noise_rate = noise_rate
        self.device = device
        self.instances_per_class = ins_per_class
    ...
    def __getitem__(self,index):
        ...
        for i,name in enumerate(class_name):
            cl = self.json_data[name][:self.instances_per_class]
        ...
        if torch.cuda.is_available():support_label,query_label=support_label.cuda(self.device),query_label.cuda(self.device)
